...
class LatentDecoupleModel(nn.Module):
    ...
    def __init__(
        ...
        self.style_gate = nn.Sequential(
            nn.Conv2d(16, 64, 3, padding=1),  # zt 4 + z0 4 + v_style + v_content = 16
            nn.SiLU(),
            nn.Conv2d(64, 1, 3, padding=1),
        )

        if use_dit:
            in_dim = 12 if use_noise else 8
            # DiT returns [B, 4, H, W]
    ...
    def forward(self, source_images, target_images, prompts):
        ...
        z0 = source_latents
        z1 = target_latents
        noise = torch.randn_like(z0) * 0.01
        ...
        return {"pred_velocity": pred_v, "target_velocity": target_velocity, "style_velocity": pred_vs*self.style_strength, "content_velocity": pred_vc, "source_latents": source_latents}
    ...
    def sample(
        ...
        if self.use_noise:
            noise_source = torch.randn_like(z_source) * 0.01
        ...
        for i in range(num_inference_steps):
            ...
            z_in = torch.cat([z, z], dim=0)
            z0_in = torch.cat([z_source, z_source], dim=0)
            noise = torch.cat([noise_source, noise_source], dim=0) if self.use_noise else None
            ...
            gate_uncond_input = torch.cat([z, z_source, vs_uncond, vc_uncond], dim=1)
